...
- **#18** — CDK offline synth on perms-limited creds: import Route53 zones with HostedZone.fromHostedZoneAttrib…
- **#19** — Fail-closed CI deploy via GitHub OIDC: gate every AWS step on `if: env.X != ''` where X = a repo VA…
- **#16** — preview_screenshot can hang (30s) on pages doing continuous GPU/animation work: an autoplaying loop…
- **#7** — Mobile-safe parallax = CSS scroll-driven animations (animation-timeline: view()/scroll()), gated on…
- **#1** — Pin the build runtime to the framework's real engines field, verified at install time - not a remem…
