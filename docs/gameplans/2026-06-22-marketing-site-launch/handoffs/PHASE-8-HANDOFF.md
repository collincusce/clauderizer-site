...
- **#17** — Split-host preflight fix: pin profile.lock.toml build/typecheck to the ABSOLUTE v24 npm/npx path. T…
- **#1** — Pin the build runtime to the framework's real engines field, verified at install time - not a remem…
- **#12** — On an Astro static site, use vanilla Three.js + GSAP in lazy client islands rather than React Three…
